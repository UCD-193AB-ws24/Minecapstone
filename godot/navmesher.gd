...
func _generate_navmesh() -> void:
	...
	NavigationServer3D.parse_source_geometry_data(self.navigation_mesh, navmesh_baker, $/root/World)
...
# navmesher.gd:29 @ _generate_navmesh(): Source geometry parsing for navigation mesh baking had to parse RenderingServer meshes at runtime.
# This poses a significant performance issues as visual meshes store geometry data on the GPU and transferring this data back to the CPU blocks 
# the rendering. For runtime (re)baking navigation meshes use and parse collision shapes as source geometry or create geometry data procedurally in scripts.