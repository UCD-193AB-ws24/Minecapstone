...
\begin{abstract}
    ...
    Our approach enables researchers to test various embodied LLM implementations in a flexible simulated environment and contribute to creating evaluative measures for Trustworthy AI.
    \begin{figure}[ht!]
        \centering
...
The memory system represents our attempt to implement chain-of-thought reasoning, a common technique for sequential LLM decision making, as future context include the results of the agent's actions.
...
This technique focuses the LLM on reasoning and decision making away from low-level implementation details.
In the event broken or uncompilable code is generated, we catch broken code and are able to either re-prompt with the error for self-fixing behavior, or alternatively count the iteration as a failure.
...
\begin{figure}
    ...
    \begin{tabular}{lccc}
        ...
        \bottomrule
    \end{tabular}
    \caption{Performance comparison of different LLM models across test scenarios\answerTODO{Replace placeholder values}}
...
\answerTODO{Before we can rely on 
